...
=item logask (?-opts, ?start, ?end, condition)
...
Special C<at>('-s',0) form means automatic win32 service startup at operating system startup.
...
Run C<-runarg> entry after user switched (C<-surun>, C<-s> option of C<at>).
...
Switch user and execute C<-runsu> C<-runarg> (C<-s> option of C<at>).
