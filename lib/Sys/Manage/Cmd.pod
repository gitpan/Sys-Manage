...
(see also C<-cline> and C<-i>, C<-k>, C<-l>, C<-o>, C<-r>, C<-t>, C<-u>, C<-x>, C<-v>, C<-g>)
...
(see also C<-cline> and C<-assoc>)
	...
	cmdstat		- display current status using '-log'
...
Consider 'C<cmdstat>' embedded command to display statistics.
...
['C<cmdstat>'] - display current status.
