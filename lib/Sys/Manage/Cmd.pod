...
=item Loops, Redoes, Assignments, Ping
...
Assignment is a single (re)execution of command for all not affected targets.
...
[C<-cassign> and C<-ping>, C<-credo>, C<-cloop>]
...
through L<Sys::Manage::CmdFile|Sys/Manage/CmdFile> or C<cmdfile>.
...
=item cmdfile (script file, command file)

=item cmdfile (script file, command file => history file)

=item cmdfile (script file, command file, ? history file, ? redirect log file)
...
This is an utility method from L<Sys::Manage::CmdFile|Sys/Manage/CmdFile>
...
Executes C<-cline> for each C<-ctarget> expanded using C<-target> and C<-cxtgt>.
